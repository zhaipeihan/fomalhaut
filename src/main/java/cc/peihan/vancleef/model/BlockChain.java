package cc.peihan.vancleef.model;


import cc.peihan.vancleef.action.Pow;
import cc.peihan.vancleef.exception.OperateFailedException;
import cc.peihan.vancleef.exception.VerifyFailedException;
...
public class BlockChain {
    ...
    public boolean isBlockChainValid(List<Block> blocks) throws ServiceException {
        ...
        //只有创世区块
            ...
            if (!block.getPreviousHash().startsWith("0")) {
                return false;
            }
        } else {
            //从创世区块之后的第一个区块开始
    ...
    public void consensus(List<Block> otherNodeBlocks) throws ServiceException {
        ...
        //每次都使用最长链替换当前节点
        for (int i = otherNodeBlocks.size() - 1; i >= 0; i--) {
